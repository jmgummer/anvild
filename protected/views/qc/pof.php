	...
	<div role="content">
		<div class="widget-body no-padding crypton">
			...
			<fieldset>
				<div class="clearfix">
					<div class="col-md-6">
						<div class="qc_section">
							...
							<?php echo $form->dropDownList($model, 'reportformat', array(
											'0'=>'Standard',
											'1'=>'Group by Stations (Break Down by Brand)',
											'2'=>'Group by Brands (Break Down by Station)',
											// '3'=>'Group by Ad Types (Break Down by Station)'
										), array('class'=>'form-control', 'name'=>'reportformat','id'=>'reportformat')); ?>
						</div>
					</div>
					<div class="col-md-6">
						<div class="qc_section">
				...
				<div class="qc_section clearfix" style="height: 300px; overflow-y: auto">
					<header role="heading"><strong>TV Station(s) &nbsp;<a onClick="select_all('tvstation', '1');">Check All TV Stations</a> | <a  onClick="select_all('tvstation', '0');">Uncheck All Stations</a></strong> </header>
					<?php echo $form->checkBoxList($model,'tv_station', Station::AdminTvStations(), array('name'=>'tvstation')); ?>
				</div>
				<div class="qc_section clearfix" style="height: 300px; overflow-y: auto">
					<header role="heading"><strong>Radio Station(s) &nbsp;<a onClick="select_all('radiostation', '1');">Check All Radio Stations</a> | <a  onClick="select_all('radiostation', '0');">Uncheck All Stations</a></strong> </header>
					<?php echo $form->checkBoxList($model,'radio_station', Station::AdminRadioStations(), array('name'=>'radiostation')); ?>
				</div>
...
<div class="">
	...
	if(isset($_POST['startdate']) && isset($_POST['enddate'])){
		...
		if(isset($_POST['tvstation']) && !empty($_POST['tvstation'])){
			foreach ($_POST['tvstation'] as $stationkey) {
				$stations[] = $stationkey;
			}
			$set_stations1 = implode(',', $stations);
		}

		if(isset($_POST['radiostation']) && !empty($_POST['radiostation'])){
			foreach ($_POST['radiostation'] as $stationkey) {
				$stations[] = $stationkey;
			}
			$set_stations2 = implode(',', $stations);
		}

		if($set_stations1 == "" && $set_stations2 == ""){
			$error_message = "Please Select Station(s)";
		} else{
			$set_stations = $set_stations1 .",". $set_stations2;
			$set_stations = rtrim($set_stations,",");
			$set_stations = ltrim($set_stations,",");
		}
	...
	if(isset($_POST['startdate']) && $error_message==''){
		$logs = new QCPOF;
		$stationlog = $logs->GetLogs($set_stations,$set_brands,$set_adtypes,$startdate,$enddate,$reportformat,$company);
	}else{
		echo $error_message;
	}
