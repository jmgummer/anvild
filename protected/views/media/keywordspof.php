...
$pdf_file = AnvilPDF::StandardKeywordsPDF($temp_table,'POF Log',$linkurl,$currency,$adtypes);
$excel_file = AnvilExcel::StandardKeywordExcel($temp_table,'POF Log',$linkurl,$currency,$adtypes);
            ...
            if($found_adtypes = Yii::app()->db3->createCommand($adtypes)->queryAll()){
                foreach ($found_adtypes as $adkey) {
                    ...
                    if($data = Yii::app()->db3->createCommand($union_select)->queryAll())
                    {
                        ...
                        echo '<thead><th></th><th>Date</th><th>Day</th><th>Time</th><th>Ad Name</th><th>Brand Name</th><th>Type</th><th>Duration(h:m:s)</th><th>Comment</th><th>Rate('.$currency.')</th><th>Good Words</th><th>Bad Words</th></thead>';
                        $sum = 0;
                        foreach ($data as $result) {
                            ...
                            if($entry_identifier==2){
                                ...
                                if($goodentries=MentionKeyword::model()->findAllBySql($goodmentionkeysql)){
                                    ...
                                    echo "<td>$entrycount/$totalgoodwords_count <small><br>$analogy</small></td>";
                                }else{
                                    echo "<td>0/$totalgoodwords_count</td>";
                                }
                                unset($listarray);
                                ...
                                if($badentries=MentionKeyword::model()->findAllBySql($badmentionkeysql)){
                                    ...
                                    $analogy.= implode(', ', $listarray);
                                    echo "<td>$entrycount/$totalbadwords_count <small><br>$analogy</small></td>";
                                }else{
                                    echo "<td>0/$totalbadwords_count</td>";
                                }
                                unset($listarray);
                            }
                            // End Djmention Tings
