...
* @since       July 2008
*/
$this->pageTitle=Yii::app()->name.' | Proof of Flight - Call in';
$this->breadcrumbs=array('Proof of Flight(Electronic)'=>array('media/pofback'));

/* Print The Top */
echo '<h3>Proof of Flight Report - Call in</h3>';
...
for ($x=$year_start;$x<=$year_end;$x++)
{
    ...
    for ($y=$month_start_count;$y<=$month_end_count;$y++)
    {
        ...
        FROM '.$temp_table_month.', brand_table, djmentions_entry_types, station, mention_callin
        WHERE '.$temp_table_month.'.'.$station_query.' 
        AND mention_callin.dj_auto_id='.$temp_table_month.'.auto_id 
        AND mention_callin.callin=1
        AND '.$ad_query.'
...
$excel_file = Back2Back::CallinExcel($temp_table,'POF Log',$linkurl,$currency,$adtypes,$excludebrands);
...
FROM $temp_table 
INNER JOIN station ON station.station_id = $temp_table.station_id 
WHERE $temp_table.brand_id IN ($excludebrands) order by station.station_name asc";
if($stored_stations = Yii::app()->db3->createCommand($station_sql)->queryAll())
{
    ...
    echo '<thead><th>#</th><th>Station</th><th>Date</th><th>Day</th><th>Time</th><th>Ad Name</th><th>Brand Name</th><th>Type</th><th>Duration(h:m:s)</th><th>Comment</th><th>Rate('.$currency.')</th></thead>';
    foreach ($stored_stations as $found_stations) {
        ...
        if($found_adtypes = Yii::app()->db3->createCommand($adtype_query)->queryAll()){
            foreach ($found_adtypes as $adkey) {
                ...
                $union_select = "SELECT * 
                FROM $temp_table 
                WHERE station_id=$fstation_id AND entry_type='$adname' 
                AND $temp_table.brand_id IN ($excludebrands) 
                order by date, time";
                    ...
                    foreach ($data as $result) {
                        ...
                        // }else{
                            ...
                            if($fstation_type=='radio'){
                                ...
                                $media_link = 'http://'.$linkurl.'.reelforge.com/'.$data_this_file_path;
