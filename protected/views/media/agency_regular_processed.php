...
/* Print The Top */
echo '<h3>Proof of Flight Report (Print)</h3>';
...
if(isset($_POST['company'])){
    $agency_company_id = $_POST['company'];
}else{
    $error_code = 4;
}
...
brand_table.company_id='$agency_company_id' and
...
//         print_table, brand_table where
//         brand_table.brand_id=print_table.brand_id and
//         brand_table.company_id IN (select distinct company_id from brand_table, agency_brand 
//         where agency_brand.agency_id='$company_id' and brand_table.brand_id=agency_brand.brand_id) and
//         print_table.media_house_id=mediahouse.media_house_id and
//         date between '$startdate' and '$enddate' $brand_query $this_media_code $entry_type_query order by date asc";
// } else {
        ...
        */
        ...
        if($excel_data = Yii::app()->db3->createCommand($temp_select1)->queryAll())
        {
            foreach ($excel_data as $elements) {
                ...
                $jurl = "http://beta.reelforge.com/anvil/reports/print_story_console/print_stream.php?url=". $this_file . "&rate=$this_ave&ad_name=$this_brand_name";
