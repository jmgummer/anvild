...
$this->pageTitle=Yii::app()->name.' | Top Spenders By Brand';
$this->breadcrumbs=array('Top Spenders By Brand'=>array('ranking/brandspenders'));
?>
...
if(isset($temp_table)){
	...
	if($recordcount = Yii::app()->db3->createCommand($countsql)->queryAll()!=0){
		$company_sql="SELECT company_name, brand_name, sum(rate) as rate  from $temp_table where 
		rate>0 group by brand_name order by rate desc ";
		if($company_query = Yii::app()->db3->createCommand($company_sql)->queryAll()){
			$excel_title = "Top Spenders by Brand";
			$excel_industry = "";
			$excel_client = $company_name;
			$title = "Graph Showing Top 20 Spenders by Brand";
			$chart_name = 'topspenders';
			$table = TopSpenders::BrandSpendsTable($title,$company_query);
			$excel = TopSpenders::TopSpendersBrandExcel($table['exceldata'],$excel_title,$excel_industry,$excel_client);
			...
			$companytitle = 'Brand Ranking';
			echo "<h5>Brands Ranked From Highest to Lowest Spenders</h5>";
			echo RenderSOVBrandChart($company_query,$companytitle,'');
			echo '</div>';
		...
		echo '<h4><strong>Top Spenders Report (Brand Ranking)</strong></h4>';
...
function RenderSOVBrandChart($array,$title,$subtitle,$chart_exists=false){
	$container_name = 'sovbrand';
	$chart = new Highchart();
	$chart->chart->renderTo = "sovbrand";
	...
	foreach ($array as $key) {  
		$chartarray[$count] = array( $key['brand_name'].'<br>'.number_format($key['rate']), (int)str_replace(',', '', $key['rate']));  
	...
	$chart->series[] = array('type' => 'pie','name' => 'Top Spenders - Brand','data' => $chartarray);
