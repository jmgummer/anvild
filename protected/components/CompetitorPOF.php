...
*/
class CompetitorPOF{
	public static function CompetitorReport($startdate,$enddate,$industry,$subindustries,$brand_query,$country_id){
		...
		$print_sql = "SELECT
        mediahouse.Media_House_List, print_table.media_house_id,print_table.auto_id as print_id, print_table.file, print_table.page,
        print_table.entry_type, print_table.col, print_table.centimeter, print_table.ave, 
        print_table.brand_id, date as this_date,brand_table.brand_name as brand, company_name
        FROM mediahouse,print_table, brand_table, user_table 
        WHERE print_table.$brand_query 
        AND brand_table.company_id = user_table.company_id AND brand_table.brand_id=print_table.brand_id AND print_table.media_house_id=mediahouse.media_house_id 
        AND date between '$sqlstartdate' AND '$sqlenddate'  order by date asc";
        $print_data = Yii::app()->db3->createCommand($print_sql)->queryAll();

		$file = CompetitorPOF::CompetitorCompanies($temp_table,$currency,$linkurl,$print_data);
		return $file;
	}

	public static function CompetitorCompanies($temp_table,$currency,$linkurl,$print_data){
		ini_set('memory_limit', '1024M');
		$PHPExcel = new PHPExcel();
        $title = "Reelforge POF Competitor Report";
		...
		->setCellValue('A1', 'Reelforge Competitor Proof of Flight Report')
        ...
        $PHPExcel->getActiveSheet()->setTitle('Competitor POF - Electronic');
        ...
        ->setCellValue('A2', 'Print Summary')
        ->setCellValue('A3', $title)

        ->setCellValue('A5', 'Ad Name')
		->setCellValue('B5', 'Brand Name')
		->setCellValue('C5', 'Date')
		->setCellValue('D5', 'Page')
		->setCellValue('E5', 'Type')
		->setCellValue('F5', 'Duration(h:m:s)')
		->setCellValue('G5', 'Rate')
		->setCellValue('H5', 'Station Type')
        ...
        $count = 6;
        foreach ($print_data as $elements) {
            $this_file = $elements['file'];

            $print_type = $elements['entry_type'];
            if($print_type == 1){
            	$papertype = 'Advertisement';
            }else{
            	$papertype = 'Notice';
            }
            ...
            ->setCellValue("B$count", $elements['brand'])
            ->setCellValue("C$count", $elements['this_date'])
            ->setCellValue("D$count", $elements['page'])
            ->setCellValue("E$count", $papertype)
            ->setCellValue("F$count", '-')
            ->setCellValue("G$count", number_format($elements['ave']))
            ->setCellValue("H$count", 'Print')
            ->setCellValue("I$count", $elements['Media_House_List'])
            ->setCellValue("J$count", $elements['company_name']);
        ...
        $PHPExcel->getActiveSheet()->setTitle('Competitor POF - Print');
        $PHPExcel->setActiveSheetIndex(0);
        ...
        $filename = 'competitor_pof_'.date("Ymdhis").'_'.$filename.'.xls';
