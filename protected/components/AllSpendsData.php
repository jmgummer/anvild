...
class AllSpendsData{
    ...
    public static function GetRecords($startdate,$enddate,$industry,$subindustries,$brand_query,$country_id){
		...
		$temp_table = AllSpendsData::POFTempTable();
		/* Djmentions */
		for ($x=$year_start;$x<=$year_end;$x++)
		{
		    ...
		    for ($y=$month_start_count;$y<=$month_end_count;$y++)
		    {
		        ...
		        (auto_id,brand_id,entry_type_id,incantation_id,station_id,date,time,comment,rate,brand_name,entry_type,incantation_name,duration,file,video_file,station_name,station_type,sub_industry_id,company_id,company_name,mediatype) 
		        SELECT 
