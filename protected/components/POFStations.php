...
class POFStations
{
	public static function POFElectronicStations()
	{
		/* 
		...
		*/
	    ...
	    $title = 'The following is a Proof of Flight Summary Report  the period: between '.$startdate.' and '.$enddate.'. Broken Down By Stations';
	    ...
	    foreach ($stored_stations as $excel_stations) {
	        ...
	        */
	        ...
	        $distinct_brands = 'SELECT distinct brand_id, brand_name FROM '.$temp_table.' WHERE station_id='.$excel_station_id.' order by date, time';
	        if($brand_select = Yii::app()->db3->createCommand($distinct_brands)->queryAll()){
	        	foreach ($brand_select as $key) {
	        		...
	        		$count = $count+1;
	        		$PHPExcel->getActiveSheet()->setCellValue("A$count", $station_brand_name);
	        		$count = $count+1;

	        		if($adtypes==1){
	        			$adtypes = 'SELECT distinct entry_type  FROM '.$temp_table.' WHERE station_id='.$excel_station_id.' and brand_id = '.$station_brand_id.' order by date, time';
	        			if($found_adtypes = Yii::app()->db3->createCommand($adtypes)->queryAll()){
	        				foreach ($found_adtypes as $adkey) {
				        		...
				        		$union_select1 = 'SELECT * FROM '.$temp_table.' WHERE station_id='.$excel_station_id.' and brand_id = '.$station_brand_id.' and entry_type="'.$adname.'" order by date, time';
						        if($excel_data = Yii::app()->db3->createCommand($union_select1)->queryAll())
						        {
						            ...
						            foreach ($excel_data as $elements) {
						                ...
						                $sum = $sum+$elements['rate'];
						                $count++;
						            }
						            $count=$count+1;
							        $PHPExcel->getActiveSheet()->setCellValue("G$count", number_format($sum));
							        $PHPExcel->getActiveSheet()->getStyle("G$count")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_GENERAL);
							        $count=$count+1;
	        			...
	        			$union_select1 = 'SELECT * FROM '.$temp_table.' WHERE station_id='.$excel_station_id.' and brand_id = '.$station_brand_id.' order by date, time';
				        if($excel_data = Yii::app()->db3->createCommand($union_select1)->queryAll())
				        {
				        	$sum = 0;
				            foreach ($excel_data as $elements) {
