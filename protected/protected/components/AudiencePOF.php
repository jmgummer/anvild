...
class AudiencePOF
{
	public function GetLogs($stations,$brands,$adtypes,$startdate,$enddate,$reportformat){
		$adsarray = array();
		...
		for ($x=$year_start;$x<=$year_end;$x++){
		    ...
		    for ($y=$month_start_count;$y<=$month_end_count;$y++)
		    {
		        ...
		        $manualqry = "INSERT INTO $temp_table (adid,station,station_id,brand_id,ad_name,brand_name,date,time,type,duration,rate,date_time,active, adtype,comment,score,file,tabletype,videofile,audience,grp)
		        ...
		        concat(file_path,'', filename) AS file,'manual' AS tabletype,'' AS videofile,audience,grp
				...
				$date_qry_manual AND $temp_table_month.active=1";
				$insertsql = Yii::app()->db3->createCommand($manualqry)->execute();

				$sampleqry = "INSERT INTO $temp_table (adid,station,station_id,brand_id,ad_name,brand_name,date,time,type,duration,rate,date_time,active, adtype,comment,score,file,tabletype,videofile,audience,grp)
				...
				concat(incantation.file_path,'', incantation.incantation_file) AS videofile,audience,grp
				...
				$date_qry_auto AND $sample_table_month.active=1";
				$insertsql = Yii::app()->db3->createCommand($sampleqry)->execute();
		    }
		}
		if($reportformat==0){
			// Select General Grouped By Station ONLY
			$excelfile = AudienceExcel::StandardExcel($temp_table,$startdate,$enddate);
		...
		if($reportformat==1){
			...
			$excelfile = AudienceExcel::StationBrandExcel($temp_table,$startdate,$enddate);
		...
		if($reportformat==2){
			...
			$excelfile = AudienceExcel::BrandStationExcel($temp_table,$startdate,$enddate);
